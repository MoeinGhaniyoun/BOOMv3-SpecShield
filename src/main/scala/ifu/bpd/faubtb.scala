...
class FAMicroBTBBranchPredictorBank(params: BoomFAMicroBTBParams = BoomFAMicroBTBParams())(implicit p: Parameters) extends BranchPredictorBank()(p)
{
 ...
 //teesec
 printf("FAUBTB TagSize: %d   OffsetSize: %d\n", tagSz.asUInt, offsetSz.asUInt);
  ...
  for (w <- 0 until bankWidth) {
    ...
    //teesec
    printf("FAUBTB Resp valid: %d  Resp: %x\n", s1_resp(w).valid, s1_resp(w).bits);
  }

 //teesec
  for (w <- 0 until nWays){
    for (i <- 0 until bankWidth){
      printf("btb offsets: btb[%d][%d] = %x meta[%d][%d] = is_br: %d tag: %x ctr: %d target: %x\n", w.asUInt, i.asUInt, btb(w)(i).offset.asUInt, w.asUInt, i.asUInt, meta(w)(i).is_br, meta(w)(i).tag.asUInt, meta(w)(i).ctr.asUInt,(s1_pc.asSInt + (i << 1).S + btb(s1_hit_ways(i))(i).offset).asUInt );
